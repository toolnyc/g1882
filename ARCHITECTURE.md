...
Blob storage uses `disablePayloadAccessControl: true` so media URLs resolve directly to Blob CDN (`https://<store>.public.blob.vercel-storage.com/...`) rather than being proxied through a serverless function. A `robustBlobFetchPlugin` (`src/plugins/robustBlobFetch.ts`) patches the default Blob handler to use the canonical `downloadUrl` from the Blob API and validate `response.ok`, working around a CDN 404 bug for filenames with special characters. Filenames are also normalized in the `beforeValidate` hook (whitespace collapsed, spaces replaced with hyphens). A diagnostic endpoint at `/next/media-audit` (admin-only) audits all media documents for URL health.
